...
void PrintOpenclInfo() {
    ...
    for (int p = 0; p < (int)numPlatforms; p++) {
        ...
        clGetPlatformInfo(
            platforms[p], CL_PLATFORM_NAME, 0, NULL, &size);
        str = new char[size];
        clGetPlatformInfo(
            platforms[p], CL_PLATFORM_NAME, size, str, NULL);
        fprintf(stderr, "\tName    = '%s'\n", str);
        delete[] str;

        clGetPlatformInfo(
            platforms[p], CL_PLATFORM_VENDOR, 0, NULL, &size);
        str = new char[size];
        clGetPlatformInfo(
            platforms[p], CL_PLATFORM_VENDOR, size, str, NULL);
        fprintf(stderr, "\tVendor  = '%s'\n", str);
        delete[] str;

        clGetPlatformInfo(
            platforms[p], CL_PLATFORM_VERSION, 0, NULL, &size);
        str = new char[size];
        clGetPlatformInfo(
            platforms[p], CL_PLATFORM_VERSION, size, str, NULL);
        fprintf(stderr, "\tVersion = '%s'\n", str);
        delete[] str;

        clGetPlatformInfo(
            platforms[p], CL_PLATFORM_PROFILE, 0, NULL, &size);
        str = new char[size];
        clGetPlatformInfo(
            platforms[p], CL_PLATFORM_PROFILE, size, str, NULL);
        ...
        status = clGetDeviceIDs(platforms[p],
                                CL_DEVICE_TYPE_ALL,
                                0,
                                NULL,
                                &numDevices);
        ...
        for (int d = 0; d < static_cast<int>(numDevices); d++) {
            ...
            clGetDeviceInfo(devices[d],
                            ...
                            NULL);
            fprintf(
                stderr, "\t\tDevice Vendor ID = 0x%04x ", ui);
            switch (ui) {
                case ID_AMD:
                    fprintf(stderr, "(AMD)\n");
                    break;
            ...
            clGetDeviceInfo(devices[d],
                            ...
                            NULL);
            fprintf(stderr,
                    "\t\tDevice Maximum Work Item Sizes = %d x "
                    "%d x %d\n",
                    sizes[0],
                    sizes[1],
                    sizes[2]);
            ...
            clGetDeviceInfo(devices[d],
                            ...
                            NULL);
            fprintf(stderr,
                    "\t\tDevice Maximum Work Group Size = %d\n",
                    size);
            ...
            for (size_t e = 0; e < strlen(extensions); e++) {
                if (extensions[e] == ' ')
                    extensions[e] = '\n';
            }
            fprintf(stderr, "%s\n", extensions);
    ...
    for (int p = 0; p < (int)numPlatforms; p++) {
        ...
        for (int d = 0; d < (int)numDevices; d++) {
                                ...
                                // we'll accept anything
            {
                iplatform = p;
                idevice = d;
                *platform_ = platforms[iplatform];
                *device_ = devices[idevice];
                deviceType = type;
                deviceVendor = vendor;
